...
export default function Flows() {
  ...
  const mCreate = useApiMutation(() => api.post('/flows', newFlow), { successMessage: 'Đã tạo Luồng', invalidate: [['flows']], onSuccess: () => setNewFlow({ name: '', code: '' }) });
  const mClone = useApiMutation((id: string) => api.post(`/flows/${id}/clone`), { successMessage: 'Đã nhân bản Luồng', invalidate: [['flows']] });
  const mUpdateFlow = useApiMutation(({ id, body }: any) => api.patch(`/flows/${id}`, body), { successMessage: 'Đã lưu Luồng', invalidate: [['flows']], onSuccess: () => setEditFlow(null) });
  const mDelete = useApiMutation((id: string) => api.delete(`/flows/${id}`).then((r) => r.data), { successMessage: (d: any) => (d?.productCount ? `Đã xoá Luồng · đã gỡ khỏi ${d.productCount} sản phẩm` : 'Đã xoá Luồng'), invalidate: [['flows'], ['products']] });
  ...
  const flowActions = (flow: any, version: any, orgNames: string[]) => canManage ? (
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      <button className="btn btn-sm btn-danger" title="Xoá Luồng" onClick={() => {
        ...
        const msg = warns.length ? `Luồng "${flow.name}" ${warns.join(' và ')}. Vẫn xoá?` : `Xoá Luồng "${flow.name}"?`;
  ...
  const orgCell = (orgNames: string[]) => orgNames.length
    ...
    : <span className="chip" style={{ fontSize: 11 }}><Building2 size={11} />Toàn Tổ Chức</span>;

  return (
    <>
      <PageHead eyebrow="Thiết kế quy trình" title="Luồng & Sự kiện" subtitle="Thiết kế quy trình truy xuất · nhấp sự kiện để cấu hình" />
      ...
      {canManage && (
        <div className="card p-5 mb-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <b className="text-[15px]">Tạo Luồng mới</b>
              <div className="text-xs text-[var(--muted)]">Gán tổ chức cho Luồng ở nút Sửa (có thể chọn nhiều tổ chức / bộ phận).</div>
            </div>
          </div>
          <div className="flex flex-wrap gap-3 items-end">
            <label className="block flex-1 min-w-[180px]"><span className="label">Tên Luồng</span><input className="input" value={newFlow.name} onChange={(e) => setNewFlow({ ...newFlow, name: e.target.value })} placeholder="Chuỗi rau sạch" /></label>
            ...
            <button className="btn btn-primary" disabled={!newFlow.name || !newFlow.code || mCreate.isPending} onClick={() => mCreate.mutate()}><Plus size={16} />Tạo Luồng</button>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2.5 mb-5 items-center">
        <div className="flex items-center gap-2 rounded-full px-4 h-11 flex-1 min-w-[220px]" style={{ border: '1px solid var(--border)', background: 'var(--bg)' }}>
          ...
          <input className="flex-1 bg-transparent outline-none text-sm" placeholder="Tìm Luồng theo tên / mã…" value={flowQ} onChange={(e) => { setFlowQ(e.target.value); setPage(1); }} />
        </div>
      ...
      {flows.isLoading ? <Spinner /> : (flows.data?.items?.length ?? 0) === 0 ? (
        <div className="card"><EmptyState title="Không có Luồng" hint={flowQ || orgFilter ? 'Thử bỏ bộ lọc.' : 'Tạo Luồng đầu tiên ở trên.'} /></div>
      ) : (
        <>
        {/* Sổ cái cho desktop */}
                ...
                return (
                  <Fragment key={flow.id}>
                    <tr>
                      ...
                      <td><span className={`pill ${version?.isPublished ? 'pill-good' : 'pill-warn'}`}><i />{version?.isPublished ? 'Đã xuất bản' : 'Nháp'}</span></td>
        ...
        {flows.data.total > flows.data.pageSize && (
          <div className="flex items-center justify-center gap-3 mt-6">
            ...
            <span className="text-sm text-[var(--muted)]">Trang {flows.data.page} / {Math.ceil(flows.data.total / flows.data.pageSize)} · {flows.data.total} luồng</span>
          ...
          <>
            ...
            {tab === 'fields' && (
              <div className="flex flex-col gap-3">
                ...
                <div className="text-[11px] font-bold uppercase tracking-wide text-[var(--faint)] mt-1">Trường thông tin tự cấu hình</div>
            ...
            {tab === 'perm' && (
              <div>
                <p className="text-sm text-[var(--muted)] mb-3">Vai trò nào được nhập sự kiện này? Quyền theo từng người được gán khi gán Luồng vào sản phẩm hoặc khi tạo lịch truy xuất.</p>
                ...
                <div className="flex gap-4 flex-wrap">
                  <div className="flex-1 min-w-[200px]">
                    ...
                    {on('event') && <>
                      ...
                      {[['who', 'Ai'], ['where', 'Ở đâu'], ['when', 'Thời gian'], ['media', 'Hình ảnh']].map(([k, l]) => (
                      ...
                      <div className="text-[11px] font-bold uppercase text-[var(--faint)] mt-3 mb-1">Thông tin tự cấu hình</div>
                      {(sel.fields ?? []).map((f: any) => (
      ...
      {/* Edit flow drawer */}
      <Drawer open={!!editFlow} onClose={() => setEditFlow(null)} title={<b>Sửa Luồng</b>}
        ...
        {editFlow && <>
          <label className="block mb-3"><span className="label">Tên Luồng</span><input className="input" value={editFlow.name} onChange={(e) => setEditFlow({ ...editFlow, name: e.target.value })} /></label>
