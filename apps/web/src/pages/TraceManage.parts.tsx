...
function StatusPillLocal({ status }: { status: string }) {
  const m: Record<string, [string, string]> = { noflow: ['pill-warn', 'Chưa có Luồng'], ready: ['pill-neutral', 'Sẵn sàng'], active: ['pill-accent', 'Đang khai báo'], overdue: ['pill-bad', 'Có lịch quá hạn'], done: ['pill-good', 'Hoàn thành'] };
...
export function TableRow({ r, index, onOpen }: { r: any; index?: number; onOpen: () => void }) {
  return (
    <tr className="cursor-pointer" onClick={onOpen}>
      ...
      <td>{r.flowCount === 0 ? <span className="text-[13px] font-semibold text-[var(--warn)]">Chưa gán</span> : <span className="num text-[15px]">{r.flowCount}</span>}</td>
      <td><span className="num text-[15px] text-[var(--ink-2)]">{r.batchCount}</span></td>
      <td><span className="num text-[15px] text-[var(--ink-2)]">{r.taskCount}</span>{r.overdue > 0 && <span className="block text-[11px] font-bold text-[var(--danger)] mono">{r.overdue} quá hạn</span>}</td>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2">
            <span className="chip"><GitBranch size={11} />{r.flowCount} luồng</span>
  ...
  return (
    <Drawer open onClose={onClose} title={<><b className="block leading-tight">{product.name}</b><span className="text-xs text-[var(--muted)] mono">GTIN {product.gtin}</span></>}>
      <div className="tabbar mb-4">
        {([['flow', 'Luồng'], ['assign', 'Phân công'], ['sched', 'Lịch'], ['progress', 'Tiến độ']] as const).map(([k, l]) => (
...
function FlowTab({ product, flows, batches, onChanged }: any) {
  ...
  const attach = useApiMutation((flowId: string) => api.post(`/products/${product.id}/flows/attach`, { flowId }), { successMessage: 'Đã gắn Luồng', invalidate: [['product', product.id], ['products']], onSuccess: () => { setAdding(false); onChanged(); } });
  const detach = useApiMutation((flowId: string) => api.delete(`/products/${product.id}/flows/${flowId}`), { successMessage: 'Đã gỡ Luồng', invalidate: [['product', product.id], ['products']], onSuccess: () => onChanged() });
  const clone = useApiMutation(async (flowId: string) => { const { data } = await api.post(`/flows/${flowId}/clone`); await api.post(`/products/${product.id}/flows/attach`, { flowId: data.id }); }, { successMessage: 'Đã nhân bản & gắn Luồng', invalidate: [['flows-all'], ['product', product.id], ['products']], onSuccess: () => onChanged() });

  const attachedIds = new Set(flows.map((f: any) => f.id));
  return (
    <div className="flex flex-col gap-3">
      <div className="text-[13px] text-[var(--muted)]">Một sản phẩm có thể gắn nhiều Luồng. Phân biệt theo lô / ngày sản xuất khi tạo lịch. QR chung dùng GTIN; QR riêng dùng theo lô.</div>
      {flows.length === 0 && <div className="card p-4"><EmptyState title="Chưa gắn Luồng" hint="Gắn Luồng có sẵn hoặc tạo mới bên dưới." /></div>}
      {flows.map((f: any) => (
        <div key={f.id} className="card p-3.5">
          <div className="flex items-center gap-2.5">
            <span className="iconbox" style={{ width: 34, height: 34 }}><GitBranch size={16} /></span>
            <div className="flex-1 min-w-0"><b className="text-sm block truncate">{f.name}</b><div className="text-xs text-[var(--muted)]">{f.versions?.[0]?.eventDefinitions?.length ?? 0} Sự kiện · {f.versions?.[0]?.isPublished ? 'đã xuất bản' : 'nháp'}</div></div>
          </div>
          <div className="flex flex-wrap gap-1.5 mt-3">
            ...
            <button className="btn btn-sm" onClick={() => clone.mutate(f.id)}><Copy size={13} />Nhân bản</button>
            <button className="btn btn-sm btn-danger" onClick={() => { if (window.confirm(`Gỡ Luồng "${f.name}" khỏi sản phẩm?`)) detach.mutate(f.id); }}><Trash2 size={13} /></button>
          </div>
        </div>
      ))}
      ...
      {adding ? (
        <div className="card p-3.5 flex flex-col gap-2">
          <div className="label" style={{ margin: 0 }}>Chọn Luồng để gắn thêm</div>
          {flowsAll.isLoading ? <Spinner /> : (
            <div className="flex flex-col gap-1.5 overflow-y-auto pr-1" style={{ maxHeight: 240 }}>
              ...
              {(flowsAll.data ?? []).filter((f) => !attachedIds.has(f.id)).length === 0 && <p className="text-sm text-[var(--muted)] py-1">Đã gắn hết Luồng hiện có.</p>}
            </div>
          )}
        ...
        <button className="btn self-start" onClick={() => setAdding(true)}><Plus size={15} />Gắn / tạo Luồng</button>
      )}
...
function NewFlowInline({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const create = useApiMutation(
    ...
    { successMessage: 'Đã tạo Luồng', onSuccess: (d) => { setName(''); onCreated(d.id); } },
  );
  return (
    <div className="flex gap-2 mt-1">
      <input className="input flex-1" placeholder="Tên Luồng mới…" value={name} onChange={(e) => setName(e.target.value)} />
...
function AssignTab({ flows }: { flows: any[] }) {
  ...
  if (flows.length === 0) return <div className="card p-4"><EmptyState title="Chưa có Luồng" hint="Gắn Luồng ở tab Luồng để phân công khai báo." /></div>;
  return (
    <div className="flex flex-col gap-3">
      {flows.length > 1 && (
...
function AssignFlow({ flow }: { flow: any }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-[13px] text-[var(--muted)]">Phân công toàn Luồng (khai báo mọi Sự kiện) hoặc chi tiết theo từng Sự kiện. Quyền chi tiết được ưu tiên khi kê khai.</div>

      <div className="card p-3.5 flex flex-col gap-2.5">
        <div className="label" style={{ margin: 0 }}>Phạm vi phụ trách</div>
        <select className="input" value={scope} onChange={(e) => setScope(e.target.value)}>
          <option value="">Toàn bộ Luồng ({flow.name})</option>
          {events.map((ev: any) => <option key={ev.id} value={ev.id}>Chỉ Sự kiện: {ev.name}</option>)}
        </select>
          ...
          {list.map((p: any) => (
            <div key={p.id} className="flex items-center gap-2.5 p-2.5 rounded-xl" style={{ border: '1px solid var(--border)' }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className={`chip ${p.eventDefinitionId ? '' : 'chip-accent'}`} style={{ fontSize: 10.5, padding: '1px 7px' }}>{p.eventDefinitionId ? `Sự kiện: ${p.eventDefinition?.name ?? '—'}` : 'Toàn bộ Luồng'}</span>
              </div>
        ...
        <div className="card p-3.5 flex flex-col gap-2.5 anim-in">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select className="input" value={f.flowId} onChange={(e) => setF({ ...f, flowId: e.target.value })}>
              <option value="">Luồng của SP</option>
...
function ProgressTab({ product, flows, batches }: { product: any; flows: any[]; batches: any[] }) {
  ...
  if (flows.length === 0) return <div className="card p-4"><EmptyState title="Chưa có Luồng" hint="Gắn Luồng để theo dõi tiến độ khai báo." /></div>;

  return (
    <div className="flex flex-col gap-3">
      <div className="text-[13px] text-[var(--muted)]">Tiến độ tính theo số Sự kiện đã có bản ghi được duyệt trên từng lô.</div>
      {records.isLoading ? <Spinner /> : lots.size === 0 ? (
        ...
        return (
          <div key={lot} className="card p-3.5">
            <div className="flex items-center justify-between mb-2"><b className="text-sm"><Layers size={13} className="inline mr-1" />{lot}</b><span className="text-xs text-[var(--muted)] num">{doneSet.size}/{totalEvents} Sự kiện · {pct}%</span></div>
