...
export default function Categories() {
  ...
  return (
    <>
      <PageHead eyebrow="Danh mục" title="Danh mục & trường động" subtitle="Cấu hình trường động cho từng danh mục sản phẩm" />
        ...
        <div className="flex flex-col gap-4">
          {(list.data ?? []).map((c: any) => (
            <div key={c.id} className="card p-5" style={{ boxShadow: 'none' }}>
              ...
              <div className="eyebrow mb-2">Trường động</div>
                ...
                {(c.fields ?? []).map((fl: any) => (
                  <div key={fl.id} className="flex items-center gap-3 py-2.5">
                    <div className="flex-1"><b className="text-[13.5px]">{fl.label}</b> <span className="mono text-xs text-[var(--muted)]">{fl.key}</span>
                      <div className="text-[11.5px] text-[var(--muted)]"><span className="mono">{fl.type}</span>{fl.required ? ' · bắt buộc' : ''}{fl.publicVisible ? ' · công khai' : ''}</div></div>
                    <button className="btn btn-sm btn-danger" onClick={() => delField.mutate(fl.id)}><Trash2 size={14} /></button>
                  </div>
                ))}
                {(c.fields ?? []).length === 0 && <p className="text-sm text-[var(--muted)] py-2.5">Chưa có trường.</p>}
              </div>
              <div className="flex flex-wrap gap-2 items-end p-3 rounded-xl" style={{ background: 'var(--surface)' }}>
                <label className="block"><span className="label">Khoá</span><input className="input mono w-28" value={f(c.id).key} onChange={(e) => setField({ ...field, [c.id]: { ...f(c.id), key: e.target.value } })} /></label>
                ...
                <button className="btn" disabled={!f(c.id).key || !f(c.id).label}
                  onClick={() => addField.mutate({ id: c.id, f: f(c.id) })}><Plus size={15} />Thêm trường</button>
              </div>
            </div>
          ))}
        </div>
