          ...
          <div className="flex flex-col gap-2 overflow-y-auto pr-1" style={{ maxHeight: 240 }}>
            {shownUsers.map((u: any) => (
              <button key={u.id} onClick={() => patch({ assigneeId: u.id })} className={`opt ${draft.assigneeId === u.id ? 'sel' : ''}`}>
                <span className="w-9 h-9 rounded-full grid place-items-center text-white text-xs font-bold flex-none" style={{ background: 'linear-gradient(135deg,var(--accent),var(--accent-2))' }}>{u.fullName?.split(' ').slice(-1)[0]?.[0] ?? '?'}</span>
...
function NewFlow({ onCreated }: { onCreated: (id: string) => void }) {
  const toast = useToast();
...
function AddEvent({ versionId, onAdded }: { versionId?: string; onAdded: () => void }) {
  const toast = useToast();
  const [open, setOpen] = useState(false);
  const [name, setName] = useState('');
...
function CloneElabel({ currentId, onCloned }: { currentId?: string; onCloned: () => void }) {
  const toast = useToast();
  const [open, setOpen] = useState(false);
...
function renderVars(html: string, product: any): string {
...
function MiniRTE({ value, onChange }: { value: string; onChange: (v: string) => void }) {
