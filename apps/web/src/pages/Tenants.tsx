...
export default function Tenants() {
  ...
  return (
    <>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-[11px] uppercase tracking-wide text-[var(--faint)]">
              <th className="px-4 py-3 font-bold">Tenant</th><th className="px-4 py-3 font-bold">Mã</th>
              <th className="px-4 py-3 font-bold">Đơn vị</th><th className="px-4 py-3 font-bold">Người dùng</th><th className="px-4 py-3 font-bold">Sản phẩm</th></tr></thead>
            <tbody>
              {(list.data ?? []).map((t: any) => (
                <tr key={t.id} style={{ borderTop: '1px solid var(--border)' }}>
                  <td className="px-4 py-3 font-semibold flex items-center gap-2"><Building2 size={16} className="text-[var(--accent)]" />{t.name}</td>
                  <td className="px-4 py-3 mono text-[var(--muted)]">{t.code}</td>
                  <td className="px-4 py-3 num">{t._count?.organizations ?? 0}</td>
                  <td className="px-4 py-3 num">{t._count?.users ?? 0}</td>
                  <td className="px-4 py-3 num">{t._count?.products ?? 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Drawer open={open} onClose={() => setOpen(false)} title={<b>Tạo tenant mới</b>}
        ...
        <label className="block mb-3"><span className="label">Tên tenant</span><input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="UBND Tỉnh Lâm Đồng" /></label>
        <label className="block mb-3"><span className="label">Mã (unique)</span><input className="input mono" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="LAMDONG" /></label>
