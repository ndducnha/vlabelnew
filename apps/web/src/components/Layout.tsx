...
const SECTIONS: { title: string; items: NavItem[] }[] = [
  {
    title: 'Chung',
    items: [
      { to: '/intro', label: 'Giới thiệu', icon: Info },
      { to: '/org', label: 'Tổ chức', icon: Network, perm: PERMISSIONS.ORGANIZATION_MANAGE },
      { to: '/users', label: 'Người dùng', icon: UsersIcon, perm: PERMISSIONS.USER_MANAGE },
      { to: '/products', label: 'Quản lý sản phẩm', icon: Package, perm: PERMISSIONS.PRODUCT_CREATE },
    ],
  },
  {
  ...
  return (
    <div className="min-h-screen md:grid" style={{ gridTemplateColumns: '264px 1fr' }}>
      ...
      <aside className="hidden md:flex flex-col gap-0.5 p-3 sticky top-0 h-screen"
        ...
        <button onClick={toggleTheme} className="flex items-center gap-2.5 px-2.5 py-2.5 rounded-[10px] text-[14.5px] font-medium text-[var(--ink-2)] hover:bg-[var(--surface)]">
        ...
        <div className="flex items-center gap-2.5 p-2 rounded-xl mt-1" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => { logout(); nav('/login'); }} title="Đăng xuất"><LogOut size={16} /></button>
        </div>
      </aside>
