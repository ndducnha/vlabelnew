...
import { Card, Button, Loading, Empty, Pill, ProgressBar, SegTabs, Avatar, AppText } from '../../components/ui';
...
export default function ProductDetailScreen({ route, navigation }: any) {
  ...
  const detail = useQuery({ queryKey: ['product', productId], queryFn: () => api.get(`/products/${productId}`).then((r) => r.data) });
  const flowsAll = useQuery({ queryKey: ['flows-all'], queryFn: () => api.get('/flows').then((r) => r.data) });
  const flows = useMemo(() => {
    const ids = new Set((detail.data?.flows ?? []).map((x: any) => x.flow?.id ?? x.flowId));
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: t.surface }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      <Card style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        <View style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: t.accentSoft, alignItems: 'center', justifyContent: 'center' }}><Ionicons name="cube-outline" size={22} color={t.accent} /></View>
        <View style={{ flex: 1 }}><Text style={{ color: t.ink, fontWeight: '800', fontSize: 16 }}>{name}</Text><Text style={{ color: t.muted, fontSize: 12 }}>GTIN {gtin}</Text></View>
        <Button title="Hành trình" small variant="default" onPress={() => navigation.navigate('Journey', { productId, name, gtin })} />
      </Card>

      <View style={{ marginBottom: 14 }}>
        <SegTabs value={tab} onChange={setTab} options={[{ value: 'flow', label: 'Flow' }, { value: 'assign', label: 'Phân công' }, { value: 'sched', label: 'Lịch' }, { value: 'progress', label: 'Tiến độ' }]} />
      </View>

      {detail.isLoading ? <Loading /> : (
        <>
          {tab === 'flow' && <FlowTab productId={productId} gtin={gtin} flows={flows} attachedIds={new Set(flows.map((f: any) => f.id))} flowsAll={flowsAll.data ?? []} onChanged={() => detail.refetch()} navigation={navigation} />}
...
function FlowTab({ productId, gtin, flows, attachedIds, flowsAll, onChanged, navigation }: any) {
  ...
  const attach = useMutation({ mutationFn: (flowId: string) => api.post(`/products/${productId}/flows/attach`, { flowId }), onSuccess: () => { toast('Đã gắn Flow'); inv(); }, onError: (e) => toast(apiError(e), false) });
  const detach = useMutation({ mutationFn: (flowId: string) => api.delete(`/products/${productId}/flows/${flowId}`), onSuccess: () => { toast('Đã gỡ Flow'); inv(); }, onError: (e) => toast(apiError(e), false) });

  return (
    <View style={{ gap: 10 }}>
      {flows.length === 0 && <Empty title="Chưa gắn Flow" hint="Gắn Flow bên dưới." />}
      {flows.map((f: any) => (
        <Card key={f.id}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <Ionicons name="git-branch-outline" size={18} color={t.accent} />
            <View style={{ flex: 1 }}><Text style={{ color: t.ink, fontWeight: '700' }}>{f.name}</Text><Text style={{ color: t.muted, fontSize: 12 }}>{f.versions?.[0]?.eventDefinitions?.length ?? 0} công đoạn</Text></View>
          </View>
      ...
      <AppText muted>Gắn thêm Flow (một QR nhiều Flow):</AppText>
      {flowsAll.filter((f: any) => !attachedIds.has(f.id)).map((f: any) => (
        <Card key={f.id} onPress={() => attach.mutate(f.id)} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <Text style={{ flex: 1, color: t.ink, fontWeight: '600' }}>{f.name}</Text>
        </Card>
      ))}
  ...
  return (
    <Modal transparent animationType="fade" onRequestClose={onClose}>
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,.5)', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <View style={{ backgroundColor: t.card, borderRadius: 18, padding: 20, alignItems: 'center', width: 280 }}>
          <Text style={{ color: t.ink, fontWeight: '800', marginBottom: 12 }}>QR sản phẩm</Text>
          {qr.isLoading ? <Loading /> : <Image source={{ uri: qr.data?.dataUrl }} style={{ width: 220, height: 220 }} />}
          <Text style={{ color: t.muted, fontSize: 11, marginTop: 10, textAlign: 'center' }}>{qr.data?.url}</Text>
        </View>
      </Pressable>
    </Modal>
  );
...
function AssignTab({ flows }: any) {
  ...
  if (!flow) return <Empty title="Chưa có Flow" hint="Gắn Flow để phân công." />;
  return (
    <View style={{ gap: 12 }}>
      ...
      <Card style={{ gap: 10 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
          <Pressable onPress={() => setScope('')}><Pill label="Toàn Flow" tone={scope === '' ? 'accent' : 'neutral'} /></Pressable>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1, borderColor: t.border, borderRadius: 10, paddingHorizontal: 10 }}>
          ...
          <TextInput style={{ flex: 1, color: t.ink, paddingVertical: 9 }} placeholder="Tìm người…" placeholderTextColor={t.faint} value={q} onChangeText={setQ} autoCapitalize="none" />
        </View>
        {(search.data ?? []).map((u: any) => (
          <Pressable key={u.id} onPress={() => add.mutate(u.id)} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 }}>
            ...
            <Text style={{ flex: 1, color: t.ink }}>{u.fullName}</Text>
      ...
      {perms.isLoading ? <Loading /> : (perms.data ?? []).map((p: any) => (
        <Card key={p.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <View style={{ flex: 1 }}><Text style={{ color: t.ink, fontWeight: '700' }}>{p.user.fullName}</Text><Pill label={p.eventDefinitionId ? `Công đoạn: ${p.eventDefinition?.name ?? '—'}` : 'Toàn bộ Flow'} tone={p.eventDefinitionId ? 'neutral' : 'accent'} /></View>
          <Pressable onPress={() => del.mutate(p.id)}><Ionicons name="trash-outline" size={18} color={t.danger} /></Pressable>
        </Card>
      ))}
    </View>
  );
...
function SchedTab({ productId, flows }: any) {
  ...
  const list = (tasks.data ?? []).filter((x: any) => x.product?.id === productId);

  return (
    <View style={{ gap: 10 }}>
      {tasks.isLoading ? <Loading /> : list.length === 0 ? <Empty title="Chưa có lịch" hint="Tạo lịch trên web hoặc Helper." /> : list.map((x: any) => {
        const od = isOverdue(x.endDate, x.status);
        return (
          <Card key={x.id}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={{ flex: 1, color: t.ink, fontWeight: '700' }}>{x.name || 'Lịch truy xuất'}</Text>
            ...
            <Text style={{ color: t.muted, fontSize: 12, marginTop: 6 }}>Hạn {vnDate(x.endDate)}</Text>
...
function ProgressTab({ productId, flows }: any) {
  ...
  const doneSet = new Set(['APPROVED', 'LOCKED']);
  const lots = useMemo(() => {
    const m = new Map<string, Set<string>>();
    for (const r of records.data ?? []) if (doneSet.has(r.status)) { const lot = r.traceableItem?.batchOrLot || '(không lô)'; const s = m.get(lot) ?? new Set(); s.add(r.eventDefinitionId); m.set(lot, s); }
    return Array.from(m.entries());
  }, [records.data]);

  if (flows.length === 0) return <Empty title="Chưa có Flow" />;
  return (
    <View style={{ gap: 10 }}>
      {records.isLoading ? <Loading /> : lots.length === 0 ? <Empty title="Chưa có dữ liệu" hint="Tiến độ hiện khi bắt đầu kê khai." /> : lots.map(([lot, set]) => {
        ...
        return (
          <Card key={lot}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}><Text style={{ color: t.ink, fontWeight: '700' }}>Lô {lot}</Text><Text style={{ color: t.muted, fontSize: 12 }}>{set.size}/{total} · {pct}%</Text></View>
